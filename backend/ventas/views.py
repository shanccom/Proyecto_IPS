...
@permission_classes([IsAuthenticated])
@permission_classes([IsAdminUser])  # Solo staff
def update_empleado(request, emplCod):
    try:
        empleado = Empleado.objects.get(emplCod=emplCod)

        # Actualizar campos si vienen en el request
        empleado.emplNom = request.data.get('emplNom', empleado.emplNom)
        empleado.emplCarg = request.data.get('emplCarg', empleado.emplCarg)
        empleado.emplCond = request.data.get('emplCond', empleado.emplCond)
        ...
        logger.error(f"Error al actualizar empleado: {str(e)}")
        return Response({
            "error": "Ocurrió un error al actualizar empleado",
            "details": str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
